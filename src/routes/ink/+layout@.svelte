<script lang="ts">
	...
	import { LayoutDashboard, FileText, Search, ScrollText, Info, Menu, X, Keyboard, Sun, Moon, Monitor } from 'lucide-svelte';
	...
	let isEditorPage = $derived(currentPath.match(/\/ink\/documents\/[^/]+$/) !== null);

	const navItems = [
		{ href: '/ink', label: 'Dashboard', icon: LayoutDashboard },
		{ href: '/ink/documents', label: 'Documents', icon: FileText },
		{ href: '/ink/search', label: 'Search', icon: Search },
		{ href: '/ink/releases', label: 'Releases', icon: ScrollText },
		{ href: '/ink/overview', label: 'Overview', icon: Info }
	];

	function isActive(href: string): boolean {
		if (href === '/ink') return currentPath === '/ink';
...
<QueryClientProvider client={queryClient}>
	{#if !authChecked}
		<div class="min-h-screen flex items-center justify-center bg-surface">
			<div class="animate-pulse text-text-theme-secondary">Loading...</div>
		</div>
	...
	{:else}
		<div class="min-h-screen flex flex-col bg-surface text-text-theme-primary">
			...
			<header class="h-10 bg-surface-elevated border-b border-theme flex items-center px-4 flex-shrink-0 z-40">
				<!-- Brand -->
				<a href="/ink" class="flex items-center gap-1.5 mr-6 flex-shrink-0">
