<script lang="ts">
	...
	function missingFilterUrl(key: string): string {
		switch (key) {
			case 'no_description': return '/ink/documents?has_description=false';
			case 'no_subjects': return '/ink/documents?has_subjects=false';
			case 'no_embedding': return '/ink/documents?has_embedding=false';
			case 'no_pdf': return '/ink/documents?has_pdf=false';
			default: return `/ink/documents?has_${key.replace('no_', '')}=false`;
		}
	}
</script>
...
<div class="space-y-4">
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center gap-2">
			<a href="/ink/documents?sort=updated_at&direction=desc" class="btn btn-outline text-xs py-1 px-2.5">
				<ClipboardList size={14} />
				Recent edits
			</a>
			<a href="/ink/search" class="btn btn-primary text-xs py-1 px-2.5">
				<Search size={14} />
				Search
			</a>
		</div>
	...
	{:else if $dashboardQuery.data}
		...
		<div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
			...
			<a href="/ink/documents?enable_status=enabled" class="card p-3 hover:shadow-md transition-shadow">
				<div class="flex items-center gap-2.5">
					<div class="w-8 h-8 rounded bg-success-light flex items-center justify-center flex-shrink-0">
			...
			<a href="/ink/documents?enable_status=disabled" class="card p-3 hover:shadow-md transition-shadow">
				<div class="flex items-center gap-2.5">
					...
					{#each Object.entries(data.health_distribution) as [key, count]}
						...
						<div>
							...
							<div class="h-1.5 bg-surface-secondary rounded-full overflow-hidden">
								<div class="{healthColor(key)} h-full rounded-full transition-all" style="width: {pct}%"></div>
							</div>
						</div>
					{/each}
				</div>
		...
		{#if data.recent_additions.length > 0}
			<div class="card">
				<div class="flex items-center justify-between px-4 py-2.5 border-b border-theme">
					...
					<a href="/ink/documents?sort=created_at&direction=desc" class="text-xs text-interactive hover:underline">
						View all
					</a>
				</div>
